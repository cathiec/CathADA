...
typedef enum{CONCRETE, ABSTRACT, IMPACT} check_mode;
...
public:
        ...
        while(NEXT.size() > 0)
        {
            ...
            if(is_sat(p_CURRENT->_e, false, &ce_solver))
            {
                    ...
                    back_track = back_track->_up)
                {
                    ...
                    /*std::cout << "##########################################################" << std::endl;
                    std::cout << latest_interpolant.simplify() << std::endl;
                    std::cout << "##########################################################" << std::endl;*/
    ...
    z3::expr impact_post(const z3::expr & before, const transition_group & tg, int step) const
    {
        ...
        for(int i = 0; i < cp.num_args(); i++)
            if(cp.arg(i).is_const())
                return cp.arg(i);
        return parse("false");
    }

    bool is_empty_impact_mode(bool print = false) const
    {
        z3::expr init = parse(_i);
        if(is_sat(init))
        {
        ...
        while(NEXT.size() > 0)
        {
            ...
            if(is_sat(p_CURRENT->_e, false, &ce_solver))
            {
                ...
                for(int i = symbols.size() - 1; i >= 0; i--)
                {
                    z3::expr current_psi = get_psi(current, _g[symbols[i]], current_step);
                    psi.push_back(current_psi);
                    whole_psi = whole_psi && current_psi;
                    //std::cout << _g[symbols[i]]._symbol << "(" << current << ") = ";
                    current = concrete_post(current, _g[symbols[i]], current_step++);
                    //std::cout << current << std::endl;
                }
                if(is_sat(whole_psi, true, &ce_solver))
                {
                    if(print)
                    {
                        std::cout << std::endl << "Example is good." << std::endl;
                ...
                {
                    std::vector<z3::expr> interpolants;
                    z3::expr latest_interpolant = parse("true");
                    for(int i = 0; i < psi.size() - 1; i++)
                    {
                        z3::expr e1 = latest_interpolant && psi[i];
                        z3::expr e2 = psi[i + 1];
    ...
    bool is_empty(check_mode m = ABSTRACT, bool print = false) const
    {
        if(m == ABSTRACT)
            ...
            return is_empty_impact_mode(print);
    }
